...
HandleIH::HandleIH(DWORD ProcessID) {
    ...
    MainHandle = OpenProcess(PROCESS_VM_OPERATION | PROCESS_VM_WRITE | PROCESS_VM_READ | PROCESS_QUERY_INFORMATION |
...
void HackIH::WriteLog(const std::string& Output) const {
    if (LogStream) {
        ...
        if (localtime_s(&TimeStruct, &time) == 0)
            *LogStream << '[' << std::setw(2) << std::setfill('0') << std::to_string(TimeStruct.tm_hour) << ':'
                       << std::setw(2) << std::setfill('0') << std::to_string(TimeStruct.tm_min) << ':' << std::setw(2)
                       << std::setfill('0') << std::to_string(TimeStruct.tm_sec) << ']' << ' ' << Output << '\n';
...
void HackIH::GetProcessesInfo() {
    ...
    if (hProcessSnap == INVALID_HANDLE_VALUE) {
        WriteLog("Failed to get process info, CreateToolhelp32Snapshot returned invalid handle with error: " +
                 std::to_string(GetLastError()));
        return;
    }
    pe32.dwSize = sizeof(PROCESSENTRY32);
    if (!Process32First(hProcessSnap, &pe32)) {
        WriteLog("Failed to get process info, Process32First returned invalid handle with error: " +
                 std::to_string(GetLastError()));
        return;
    }
    do {
...
void HackIH::GetModulesInfo(DWORD PID) {
    ...
    if (hModuleSnap == INVALID_HANDLE_VALUE) {
        WriteLog("Failed to get modules info, CreateToolHelp32Snapshot returned invalid handle with error: " +
                 std::to_string(GetLastError()));
        return;
    }
    me32.dwSize = sizeof(MODULEENTRY32);
    if (!Module32First(hModuleSnap, &me32)) {
        WriteLog("Failed to get modules info, Module32First returned invalid handle with error: " +
                 std::to_string(GetLastError()));
        return;
    }
    do {
...
void* HackIH::GetPointerAddress(const PointerIH& Pointer) const {
    ...
    for (int i = 1; i < Pointer.size(); i++) {
        std::size_t buffer = 0;
        ReadProcessMemory(ProcHandle, reinterpret_cast<void*>(Addr), &buffer, sizeof(std::size_t), nullptr);
        Addr = buffer + Pointer[i];
    }
...
std::string HackIH::GetPointerOffset(const PointerIH& Pointer) const {
    ...
    for (auto& M : Modules) {
        if (Addr >= (std::get<0>(M)) &&
            reinterpret_cast<std::size_t>(Addr) < (reinterpret_cast<std::size_t>(std::get<0>(M)) + std::get<1>(M))) {
            auto n = std::get<2>(M).rfind('\\');
            std::stringstream stream;
            stream << std::get<2>(M).substr(n + 1);
            stream << " + 0x";
            stream << std::setw(8) << std::setfill('0') << std::hex
                   << reinterpret_cast<std::size_t>(std::get<0>(M)) - reinterpret_cast<std::size_t>(Addr);
            return stream.str();
        }
...
bool HackIH::WriteRaw(const PointerIH& Pointer, const void* Buffer, std::size_t BufSize) const {
    ...
    if (!WriteProcessMemory(ProcHandle, Addr, Buffer, BufSize, 0)) {
        ...
        WriteLog("Error in writing memory (Error code: " + std::to_string(GetLastError()) + ") on Address: " +
                 stream.str());
        return false;
    }
...
bool HackIH::ReadRaw(const PointerIH& Pointer, void* OutBuffer, std::size_t ReadSize) const {
    ...
    ReadProcessMemory(ProcHandle, Addr, OutBuffer, ReadSize, 0);
...
HANDLE HackIH::CreateThread(const PointerIH& Pointer, void* Parameter, bool Suspended) const {
    ...
    HANDLE Handle = CreateRemoteThread(ProcHandle, NULL, NULL, static_cast<LPTHREAD_START_ROUTINE>(Addr), Parameter,
    ...
    switch (WaitCode) {
        ...
        case 0x0:
            ...
            if (!GetExitCodeThread(Thread, &ThreadStatus))
                WriteLog("Couldn't get status of the thread, improbable error, possibly the injection has succedeed?");
